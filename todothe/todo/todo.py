...
def index():
    ...
    c.execute(
        ...
        'FROM todo t INNER JOIN user u on t.created_by = u.id where t.created_by = %s order by created_at desc',(g.user['id'],)
    )
    todos = c.fetchall()
...
def get_todo(id):
    db, c = get_db()
    c.execute(
        'SELECT t.id, t.description, t.completed, t.created_by,t.created_at, u.username FROM todo t INNER JOIN user u on t.created_by = u.id WHERE t.id = %s',
        (id,)
    )
    todo = c.fetchone()
